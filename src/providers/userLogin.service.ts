        ...
        if (cognitoUser != null) {
            cognitoUser.getSession(function (err, session) {
                if (err) {
                    ...
                    callback.isLoggedInCallback(err, false,cognitoUser);
                }
                else {
                    console.log("Session is valid: " + session.isValid());
                    callback.isLoggedInCallback(err, session.isValid(),cognitoUser);
                }
            });
        } 
        else 
        {
            callback.isLoggedInCallback("Can't retrieve the CurrentUser", false,cognitoUser);
        }
    }
}